...
- Lógica: `src/engine/ticketParser.ts` (texto → productos) y `planner.ts` (despensa + preferencias → planes). El scoring combina cobertura de despensa + encaje con el objetivo; `assembleWeek` elige la combinación de recetas que da en las calorías del día (sin inflar raciones). `pdfText.ts` extrae texto de PDF; `documentImport.ts` importa documentos; `aiRecipes.ts` genera recetas con la API de Claude por `fetch` (no el SDK, que no compila en Hermes). Las recetas de IA viven en `aiRecipes` del contexto y se resuelven con `getRecipe`.
